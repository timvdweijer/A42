...
"""
Fact = 42018.82258451103 ################################################################import right value and sign
...
for i in np.nditer(x):
    if i == 0:
        ...
        T.append(Tq)                                        
        xx.append(i)
    elif i == .554:                                         #at hinge two where p and Fact also work
        ...
        T.append(Tq + TP +Tact + T[int(i*1000) - 1])        #append value to T_lst of all components + last component
        xx.append(i)        
    else:
        Tq = (-1*q_v*lstep* (0.25*C_a - h/2.) )             #at all values except 0 and .554 add only q_acts
...
"""
area_1 = pi * (h/2)**2 /2                                   # area's of cell 1 and two
area_2 = (C_a- h/2) * h
