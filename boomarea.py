...
coordinatesz = [0] * int(constants.n_st +2)
coordinatesy = [0] * int(constants.n_st +2)
...
for i in range(1, constants.n_st + 1):#""""check of deze plus 1 klopt"""
    if (d_st* i-  d_st/2.)< l_straight:
        ...
        coordinatesz[i ] = math.sin((beta)) * constants.h/2. 
        coordinatesy[i ] = - math.cos((beta)) * constants.h/2.
        b = copy.deepcopy(i)
    elif (d_st* i-  d_st/2.) < (l_semi_circle + l_straight* 2.):
        coordinatesz[i + 1] = math.cos(alpha)* (l_straight - ((d_st * i - d_st/2) - (l_semi_circle + l_straight))) - (constants.C_a - constants.h/2)  #substract the (chord - radius semi-circle) of the aileron for z_coordinate with origin at hinge line
        coordinatesy[i + 1] = math.sin(alpha)* (l_straight - ((d_st * i - d_st/2) - (l_semi_circle + l_straight)))
        print (i, l_straight - (((d_st * i - d_st/2) - (l_semi_circle + l_straight))))

coordinatesy[5] = constants.h /2.
coordinatesy[9] = - 1.* constants.h /2. 


plt.plot(coordinatesz, coordinatesy, 'ro')
plt.axis([-.484, max(coordinatesz)+.02, -.2, .2])
plt.show()
